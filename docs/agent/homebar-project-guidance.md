# Home Bar Project Guidance

Optional guidance for Codex when working in the Home Bar Android app repository.

Read this after the root `AGENTS.md`. The root `AGENTS.md` remains the source of truth for agent behavior. This file contains Home Bar-specific product scope, stack choices, data ownership rules, documentation paths, checks, and device smoke-test steps.
...
Avoid these features unless the current task explicitly asks for them:
...
## Home Bar Implementation Constraints

Follow the root `AGENTS.md` for planning, coding, communication, and verification behavior.
...
- If the current codebase lacks support required by the selected Epic, add the smallest reasonable support needed. Do not use that as a reason to build other Epics early.
...
2. Read this file.
3. Read the requested bucket file.
4. Read earlier bucket files only as needed for locked decisions and dependencies.
5. Do not implement later bucket behavior unless the current task says to.
...
Use `docs/prompts/reusable-bucket-implementation-prompt.md` when implementing one Epic from a bucket.

Use `docs/prompts/commit-report-prompt.md` or the `$commit-report` skill when asked to summarize the current diff or generate a suggested commit message without making code changes.

Do not automatically run the commit report prompt after every task unless the current task asks for it. For normal Epic implementation, use the Epic completion report rules in this file and the root `AGENTS.md`.
...
When asked to generate a commit message at any time, base it on the current diff. Inspect the diff before writing the message. Do not invent changes that are not present.

Use the 50/72 rule:

- Commit subject should be 50 characters or fewer when practical.
- Wrap body lines at 72 characters when practical.
- If the bucket or Epic name is long, keep the subject short and put bucket details in the body.
...
If a report section has no entries, write a short explicit line instead of omitting important context.

Do not run `git commit` unless explicitly asked. Suggested commit messages are for the user to review and copy.

For normal Epic implementation, the report must include:
...
## Home Bar Review Checklist

Before finishing a Home Bar task, check:
